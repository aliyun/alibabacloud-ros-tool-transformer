variable "local_subnet" {
  ...
  description = <<EOT
  {
    "Description": {
      ...
      "zh-cn": "需要和本地数据中心互通的VPC侧的网段，用于第二阶段协商。"
    },
    "Label": {
      "en": "LocalSubnet",
      "zh-cn": "需要和本地数据中心互通的VPC侧的网段"
    }
  }
  EOT
}
...
variable "remote_subnet" {
  ...
  description = <<EOT
  {
    "Description": {
      ...
      "zh-cn": "需要和VPC互通的本地数据中心侧的网段，用于第二阶段协商。"
    },
    "Label": {
      "en": "RemoteSubnet",
      "zh-cn": "需要和VPC互通的本地数据中心侧的网段"
    }
  }
  EOT
}

variable "remote_ca_cert" {
  type        = string
  description = <<EOT
  {
    "Description": {
      ...
      "zh-cn": "国密型VPN网关创建IPsec连接时，对端的CA证书。"
    },
    "Label": {
      "en": "RemoteCaCert",
      "zh-cn": "国密型VPN网关创建IPsec连接时"
    }
  }
  EOT
}
...
variable "enable_dpd" {
  ...
  description = <<EOT
  {
    ...
    "Label": {
      "en": "EnableDpd",
      "zh-cn": "是否开启DPD（对等体存活检测）功能"
    }
  }
  EOT
}

resource "alicloud_vpn_gateway_vpn_attachment" "vpn_attachment" {
...
output "internet_ip" {
  value       = alicloud_vpn_gateway_vpn_attachment.vpn_attachment.internet_ip
  description = "The gateway IP address of the IPsec connection."
}

output "vpn_attachment_id" {
  value       = alicloud_vpn_gateway_vpn_attachment.vpn_attachment.id
  description = "ID of the IPsec attachment."
}

output "peer_vpn_attachment_config" {
  // Could not transform ROS Attribute PeerVpnAttachmentConfig to Terraform attribute.
  value       = null
