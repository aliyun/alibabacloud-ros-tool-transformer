...
# up to date with the frontend sources under web/frontend. Runs only when those
...
if [ ! -d web/frontend/node_modules ]; then
  ...
  npm --prefix web/frontend ci >/dev/null 2>&1
fi

npm --prefix web/frontend run build >/dev/null 2>&1

if [ -n "$(git status --porcelain -- rostran/web/static)" ]; then
  echo "ERROR: rostran/web/static is out of date with web/frontend sources." >&2
