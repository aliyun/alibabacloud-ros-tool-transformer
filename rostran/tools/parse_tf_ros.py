...
def tf_properties_attrs(md: str):
    ...
    for i in h2:
        if i.string == 'Attributes Reference':
            try:
                li_list = i.next_sibling.next_sibling.next_sibling.next_sibling.find_all('li')
...
def handle_sub_properties(tf_resource: str, go_content: str, properties: dict, tail_1=None, tail_2=None):
    params = dict()
    ...
    for line in go_content:
        if '": {' in line:
            ...
            if tab == 3:
                ...
                logger.warning(f'{tf_resource}[{tail_2}]Level 4 sub-attributes - {param}')
    ...
    def get_pro(propers: dict, r: dict, ros_type: str = None):
        ...
        for k in propers:
            ...
            if propers[k]['Type'] in ('string', 'integer', 'number', 'boolean'):
                ...
                try:
                    get_pro(propers[k]['Schema']['*'].get('Schema'), r[k], ros_type='List')
                except KeyError:
                    pass
...
def parse_properties(tf_resource_type: str, tf_params: dict, ros_params: dict, ret: dict = {}):
    for param in tf_params:
        ...
        if tf_params[param] == {}:
            ...
            if ros_params.get(trans_param):
                if 'RosType' in ros_params[trans_param] and len(ros_params[trans_param]) > 1:
                    ros_type = ros_params[trans_param].pop('RosType')

                    r = parse_properties('', tf_params[param], ros_params[trans_param], {})
...
def parse_tf_ros(ak: str, sk: str, ros_resource: str, tf_resource: str, target_path: str):
    logger.info('Parse Terraform and ROS resource type...')
    # tf
    r_link = f'https://raw.githubusercontent.com/aliyun/terraform-provider-alicloud/master/website/docs/r/' \
    ...
    result['Properties'] = parse_properties(tf_resource, properties, ros_res['Properties'], {})
